...
class WSProxy:
    ...
    async def run(self) -> None:
        ...
        api_rqst = Request(
            "GET", path, b"", params=params, content_type="application/json"
        )
    ...
    async def __aenter__(self) -> None:
        ...
        try:
            ...
            for app_info in all_apps:
                ...
                print_fail(
                    f'The app "{self.app_name}" is not supported by the session.'
                )
                self.exit_code = 1
                return
...
@click.option(
    "-l", "--list-names", is_flag=True, help="Just print all available services."
)
    ...
    async def print_arguments():
        ...
        for service in apps:
            ...
            if has_arguments or has_envs:
                ...
                if has_arguments:
                    print(
                        "\tAvailable arguments: {0}".format(
                            service["allowed_arguments"]
                        )
                    )
                if has_envs:
                    print(
                        "\tAvailable environment variables: {0}".format(
                            service["allowed_envs"]
                        )
                    )
            else:
                print_info(
                    "Service {0} does not have customizable arguments.".format(
                        service["name"]
                    )
                )
