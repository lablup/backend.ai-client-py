...
def create(template_path):
    ...
    with Session() as session:
        try:
            template = session.TaskTemplate.create(body)
...
def get(template_id, template_format, owner_access_key):
    ...
    with Session() as session:
        try:
            template = session.TaskTemplate(template_id, owner_access_key=owner_access_key)
...
def list(list_all):
    ...
    with Session() as session:
        try:
            resp = session.TaskTemplate.list_templates(list_all)
            if not resp:
